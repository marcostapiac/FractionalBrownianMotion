...
from utils.plotting_functions import plt, gibbs_histogram_plot, plot_subplots, plot_parameter_traces, plot


def test_no_H(S=10000, muU=1., muX=1., gamma=1., X0=1., U0=0., H=0.8, N=2 ** 6, T=1e-3 * 2 ** 6,
    ...
    if not loadData:
        ...
        SH = np.power(deltaT, 2 * H) * fBn_covariance_matrix(N=N, H=H)
        invfBnCovMat = np.linalg.inv(SH)
    muUParams, alphaParams, muXParams, sigmaXParams = (0., 3.), (alpha, .001), (muX, .001), (2.1, np.power(1., 2) * 3.1)
    ...
    Thetas[:, 3] *= Thetas[:, 3]
    gibbs_histogram_plot(Thetas, burnOut, plottitle="Partially Observed MCMC Histogram", trueVals=[muU, alpha, muX, sigmaX ** 2],
