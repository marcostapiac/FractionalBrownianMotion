...
from src.CEV_multivar_posteriors_repara import posteriors, fBn_covariance_matrix
...
def test_no_H(S=20000, muU=1., muX=1., gamma=1., X0=1., U0=0., H=0.8, N=2 ** 10, T=1e-3 * 2 ** 10,
    ...
    muUParams, gammaParams, alphaParams, sigmaXParams = (0., 3.), (gamma, 1.5), (alpha, 1.5), (2.1, np.power(1., 2) * 3.1)
    ...
    for _ in tqdm(range(S)):
        theta = posteriors(muUParams=muUParams, gammaParams=gammaParams, muXParams=alphaParams,
