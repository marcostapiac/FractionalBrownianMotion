...
def get_config():
    """ Configuration file for discriminative LSTM given VP SDE model on 32-dimensional Fractional Brownian Motion """
    ...
    # LSTM parameters
    config.lstm_trained_path = project_config.ROOT_DIR + "src/evaluation_pipeline/trained_models/trained_discLSTM_fBm_VPSDE_model_H{:.3e}_T{}".format(
        config.hurst,
        config.timeDim).replace(".", "")
    config.lstm_snapshot_path = config.filename.replace("trained_models/", "snapshots/")
