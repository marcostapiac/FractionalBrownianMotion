...
def get_config():
    ...
    config.data_path = project_config.ROOT_DIR + "data/3DLnz_samples_H{}_T{}_{}Beta_{}Rho_{}Sigma_{}Diff_{}Init".format(
        str(config.hurst), config.ts_length, config.ts_beta, config.ts_rho, config.ts_sigma, config.diffusion, config.initState).replace(
        ".", "") + ".npy"
    ...
    tsmFileName = project_config.ROOT_DIR + "src/generative_modelling/trained_models/trained_rec_TSMWP_{}_incs_{}_unitIntv_3DLnz_VPSDE_model_H{:.3e}_T{}_Ndiff{}_Tdiff{:.3e}_trainEps{:.0e}_BetaMax{:.4e}_BetaMin{:.4e}_DiffEmbSize{}_ResLay{}_ResChan{}_DiffHiddenSize{}_{}Hybrid_{}Wghts_LSTM_H{}_Nlay{}_tl5".format(
        not config.isfBm, config.isUnitInterval, config.hurst,
        config.ts_length,
        config.max_diff_steps, config.end_diff_time, config.train_eps, config.beta_max, config.beta_min,
        config.temb_dim,
