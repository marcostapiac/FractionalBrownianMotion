...
def get_config():
    ...
    config.data_path = project_config.ROOT_DIR + "data/3DLnz_samples_t0{:g}_dT{:.3e}_{:.1e}Beta_{:.1e}Rho_{:.1e}Sigma_{}Diff_{}Init".format(
        config.t0, config.deltaT, config.ts_beta, config.ts_rho, config.ts_sigma, config.diffusion,
        config.initState[0]).replace(
        ".", "") + ".npy"
    ...
    # Model filepath
    mlpFileName = project_config.ROOT_DIR + "src/generative_modelling/trained_models/trained_rec_PM_MLP_{}LFac_Lnz63_{:.1e}B_{:.1e}R_{:.1e}Sg_VPSDE_model_H{:.1e}_T{}_Ndiff{}_Tdiff{:.3e}_trainEps{:.0e}_BetaMax{:.1e}_BetaMin{:.1e}_TembDim{}_EncShapes{}_tl{}".format(
        config.loss_factor, config.ts_beta, config.ts_rho, config.ts_sigma, config.hurst,
        config.ts_length,
...
config.quad_coeff, config.sin_coeff, config.sin_space_scale, config.mlp_hidden_dims, config.condupsampler_length, config.tdata_mult, config.tdata_mult).replace(".", "")

    tsmFileName = project_config.ROOT_DIR + "src/generative_modelling/trained_models/trained_rec_PM_TSM_{}LFac_Lnz63_{:.1e}B_{:.1e}R_{:.1e}Sg_VPSDE_model_H{:.1e}_T{}_Ndiff{}_Tdiff{:.3e}_trainEps{:.0e}_BetaMax{:.1e}_BetaMin{:.1e}_DiffEmbSz{}_ResLay{}_ResChan{}_DiffHdnSz{}_{}Hybd_{}Wghts_t0{:g}_dT{:.3e}_LSTM_H{}_Nly{}_tl{}".format(
        config.loss_factor, config.ts_beta, config.ts_rho, config.ts_sigma, config.hurst,
        config.ts_length,
        config.max_diff_steps, config.end_diff_time, config.train_eps, config.beta_max, config.beta_min,
        config.temb_dim,
        config.residual_layers, config.residual_channels, config.diff_hidden_size, config.hybrid, config.weightings,
        config.t0, config.deltaT, config.lstm_hiddendim, config.lstm_numlay, config.tdata_mult).replace(".", "")
