...
def get_config():
    """ Training hyperparameters for OUSDE SDE model on circle samples with 0.03 noise factor """
    ...
    config.save_freq = 50
    config.lr = 1e-3
    config.max_epochs = [20000]config.max_epochs = 1000
    ...
    # Model filepath
    mlpFileName = project_config.ROOT_DIR + "src/generative_modelling/trained_models/trained_MLP_noisy_circle_OUSDE_model_Ndiff{}_Tdiff{:.3e}_trainEps{:.0e}_TembDim{}_EncShapes{}_{}Hybd_{}Wghts".format(
    ...
    tsmFileName = project_config.ROOT_DIR + "src/generative_modelling/trained_models/trained_TSM_noisy_circle_OUSDE_model_Ndiff{}_Tdiff{:.3e}_trainEps{:.0e}_DiffEmbSz{}_ResLay{}_ResChan{}_DiffHdnSz{}_{}Hybd_{}Wghts".format(
        config.max_diff_steps, config.end_diff_time, config.train_eps, config.temb_dim,
