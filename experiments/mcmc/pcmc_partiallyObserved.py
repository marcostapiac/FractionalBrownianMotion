...
def test_no_H(S=20000, muU=1., muX=1., gamma=1., X0=1., U0=0., H=0.8, N=2 ** 7, T=1e-3 * 2 ** 7,
    ...
    plot_subplots(np.arange(0, T + deltaT, step=deltaT), np.array([Xs, Us]), np.array([None, None]),
                  np.array(["Time", "Time"]),
                  np.array(["Volatility", "Log Price"]), "Project Model Simulation")
    plt.show()
    ...
    theta = prior(muUParams=muUParams, gammaParams=alphaParams, muXParams=muXParams, sigmaXParams=sigmaXParams)
    theta[4] = H
    Thetas = [theta]
    ...
    for _ in tqdm(range(S)):
        theta = posteriors(muUParams=muUParams, gammaParams=alphaParams, muXParams=muXParams,
    ...
    Thetas[:, 3] *= Thetas[:, 3]
    gibbs_histogram_plot(thetas=Thetas, burnOut=burnOut, titlePlot="Partially Observed MCMC Histogram",
                         trueVals=np.array([muU, gamma, muX, sigmaX ** 2]),
