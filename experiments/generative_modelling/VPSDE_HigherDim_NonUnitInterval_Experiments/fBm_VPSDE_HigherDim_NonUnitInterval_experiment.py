...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassVPSDEDiffusion,
    ...
    fBm_samples = diffusion.reverse_process(dataSize=dataSize, timeDim=timeDim, timeLim=0, data=data,
                                            ...
                                            sigNoiseRatio=0.01, numLangevinSteps=1)
    true_samples = (timeDim ** hurst) * generate_fBm(H=hurst, T=timeDim, S=dataSize, rng=rng)
    evaluate_fBm_HigherDim_performance(true_samples, fBm_samples, h=hurst, td=timeDim, rng=rng, unitInterval=False)
...
if __name__ == "__main__":
    ...
    try:
        data = np.load(config.ROOT_DIR + "data/eight_hundred_thousand_fBn_samples_H{}_T{}.npy".format(h, td))[
               :numSamples // 1, :]
        data = (td ** h) * data
        data = data.cumsum(axis=1)
        try:
            file = open(
                config.ROOT_DIR + "src/generative_modelling/trained_models/trained_fBm_VPSDE_NUI_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                    td,
                    N, trainEps),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_modelling/trained_models/trained_fBm_VPSDE_NUI_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                                                       td,
                                                       N, trainEps),
