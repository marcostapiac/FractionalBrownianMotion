...
if __name__ == "__main__":
    ...
    for Tdiff in Tdiffs:
        modelFileName = config.ROOT_DIR + "src/generative_modelling/trained_models/trained_TSM_noisy_circle_OU_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}_DiffEmbSize{}_ResidualLayers{}_ResChan{}_DiffHiddenSize{}".format(
            td, N, Tdiff, trainEps, temb_dim, residual_layers, residual_channels, diff_hidden_size)
        ...
        try:
            ...
            diffusion = OUDiffusion(device="cpu", model=scoreModel, N=N, rng=rng, trainEps=trainEps, Tdiff=Tdiff)
            model = save_and_train_diffusion_model(data, model_filename=modelFileName, batch_size=BATCH_SIZE,
                                                   nEpochs=NUM_EPOCHS, lr=LR, diffusion=diffusion)

        # Given trained trained_models, compute KL divergence between final samples
