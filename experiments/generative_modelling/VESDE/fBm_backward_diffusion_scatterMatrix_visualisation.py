...
def run(config: ConfigDict) -> None:
    ...
    gif_path = "fBm_ScatterMatrix_rowIdxs{}_colIdxs{}_H{:.3e}_T{}_Ndiff{}_Tdiff{:.3e}_StdMax{:.4e}_StdMin{:.4e}_Nepochs{}".format(config.row_idxs, config.col_idxs,
                                                                                                          config.hurst,
                                                                                                          config.timeDim,
...
if __name__ == "__main__":
    ...
    for i in range(0, config.timeDim, 3):
        if i+2 == config.timeDim: config.row_idxs = np.array([i, i+1])
        elif i+1 == config.timeDim: config.row_idxs = np.array([i])
        else: config.row_idxs = np.array([i,i+1,i+2])
