...
from utils.experiment_evaluations import run_fBm_VESDE_score_error_experiment
...
def run(config: ConfigDict):
    ...
    score_errors = run_fBm_VESDE_score_error_experiment(dataSize=5000, diffusion=diffusion, scoreModel=scoreModel,
                                                        rng=rng,
                                                        config=config)
    ...
    plot_score_errors_ts(
        ...
        plot_title="L2 Score Error for fBm with $(H, T) = ({},{})$".format(config.hurst, config.timeDim))
    end_index = int(.1 * config.max_diff_steps)
    plot_score_errors_heatmap(score_errors[start_index:end_index, :],
                              plot_title="L2 Score Error for fBm with $(H, T) = ({},{})$".format(config.hurst,
                                                                                                 config.timeDim))
