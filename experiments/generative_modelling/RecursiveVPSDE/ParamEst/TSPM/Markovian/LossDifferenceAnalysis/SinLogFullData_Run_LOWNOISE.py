...
for config in [sinlog_config]:
    ...
    uniform_positions = torch.linspace(-.15, .15, true_drift.shape[0], device="cpu", dtype=torch.float32)[:,
                        np.newaxis]
...
save_path = (
            project_config.ROOT_DIR + f"experiments/results/SinLog_NewLongerDriftEvalExp_MSEs_{num_paths}NPaths_Diff01").replace(
    ".", "")
