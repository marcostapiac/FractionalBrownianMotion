...
for config in [ddimsNS_12d_config, ddimsNS_8d_config]:
    ...
    for file in os.listdir(model_dir):
        if config.scoreNet_trained_path in os.path.join(model_dir, file) and ("EE" not in file and "Trk" in file):
    ...
    bw = bws[58, :] if config.ndims == 8 else bws[31,:]
...
import pandas as pd
save_path = (project_config.ROOT_DIR + f"experiments/results/DDims_NewLongerDriftEvalExp_MSEs_{num_paths}NPaths_TRACK").replace(
            ".", "")
