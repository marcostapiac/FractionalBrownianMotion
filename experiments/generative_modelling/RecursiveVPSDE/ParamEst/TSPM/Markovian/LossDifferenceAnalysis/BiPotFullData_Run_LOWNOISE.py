...
assert bipot_config.diffusion == 0.1
...
ridge_eval = {t: np.inf for t in ["BiPot"]}
ridge_eval_true_law = {t: np.inf for t in ["BiPot"]}
...
for config in [bipot_config]:
    ...
    uniform_positions = torch.linspace(-.3, .3, all_true_states.shape[0], device="cpu", dtype=torch.float32)[:,
                        np.newaxis]
...
save_path = (
            project_config.ROOT_DIR + f"experiments/results/BiPot_NewLongerDriftEvalExp_MSEs_{num_paths}NPaths_Diff01").replace(
    ".", "")
