...
score_eval_std = {t: np.inf for t in ["SinLog"]}
score_eval_true_law_std = {t: np.inf for t in ["SinLog"]}
score_state_eval_std = {t: np.inf for t in ["SinLog"]}
score_uniform_eval_std = {t: np.inf for t in ["SinLog"]}

nad_eval_std = {t: np.inf for t in ["SinLog"]}
nad_eval_true_law_std = {t: np.inf for t in ["SinLog"]}
nad_state_eval_std = {t: np.inf for t in ["SinLog"]}
nad_uniform_eval_std = {t: np.inf for t in ["SinLog"]}

hermite_eval_std = {t: np.inf for t in ["SinLog"]}
hermite_eval_true_law_std = {t: np.inf for t in ["SinLog"]}
hermite_state_eval_std = {t: np.inf for t in ["SinLog"]}
hermite_uniform_eval_std = {t: np.inf for t in ["SinLog"]}


ridge_eval_std = {t: np.inf for t in ["SinLog"]}
ridge_eval_true_law_std = {t: np.inf for t in ["SinLog"]}
...
for config in [sinlog_config]:
    ...
    uniform_positions = torch.linspace(-12, 12, true_drift.shape[0], device="cpu", dtype=torch.float32)[:,
                        np.newaxis]
...
save_path = (
            project_config.ROOT_DIR + f"experiments/results/SinLog_NewLongerDriftEvalExp_MSEs_{num_paths}NPaths_Diff100").replace(
    ".", "")
