...
hermite_eval = {t: np.inf for t in ["QuadSinHF"]}
hermite_eval_true_law = {t: np.inf for t in ["QuadSinHF"]}
hermite_state_eval = {t: np.inf for t in ["QuadSinHF"]}
hermite_uniform_eval = {t: np.inf for t in ["QuadSinHF"]}


ridge_eval = {t: np.inf for t in ["QuadSinHF"]}
...
for config in [quadsin_config]:
    ...
    """
    all_score_drift_ests_uniform = np.full_like(true_drift.reshape((-1, config.ts_dims)), np.nan, dtype=float)
    all_nad_drift_ests_uniform = np.full_like(true_drift.reshape((-1, config.ts_dims)), np.nan, dtype=float)
    all_hermite_drift_ests_uniform = np.full_like(true_drift.reshape((-1, config.ts_dims)), np.nan, dtype=float)
    all_ridge_drift_ests_uniform = np.full_like(true_drift.reshape((-1, config.ts_dims)), np.nan, dtype=float)
