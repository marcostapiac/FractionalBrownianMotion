...
assert bipot_config.diffusion == 10.
...
for config in [bipot_config]:
    ...
    uniform_positions = torch.linspace(-4., 4., all_true_states.shape[0], device="cpu", dtype=torch.float32)[:,
                        np.newaxis]
...
save_path = (
            project_config.ROOT_DIR + f"experiments/results/BiPot_NewLongerDriftEvalExp_MSEs_{num_paths}NPaths_Diff100").replace(
    ".", "")
