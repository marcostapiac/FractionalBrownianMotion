...
def multivar_score_based_LSTM_drift(score_model,time_idx, h, c, num_diff_times, diffusion, num_paths, prev, ts_step, config,
    ...
    with torch.no_grad():
        if time_idx == 0:
            features, (h, c) = score_model.rnn(torch.tensor(prev[:, np.newaxis,:], dtype=torch.float32).to(device), None)
        else:
            features, (h, c) = score_model.rnn(torch.tensor(prev[:, np.newaxis,:], dtype=torch.float32).to(device), (h, c))
    ...
    for Nepoch in config.max_epochs[2:]:
        ...
        for i in tqdm(range(1, num_time_steps + 1)):
            ...
            true_states[:, [i], :] = true_states[:, [i - 1], :] \
                                     ...
                                     + eps
            global_mean, global_h, global_c = multivar_score_based_LSTM_drift(score_model=PM, time_idx=i - 1, h=global_h, c=global_c,num_diff_times=num_diff_times,
                                                          diffusion=diffusion,
            ...
            local_mean, local_h, local_c = multivar_score_based_LSTM_drift(score_model=PM, time_idx=i - 1,h=local_h, c=local_c,num_diff_times=num_diff_times,
                                                         diffusion=diffusion,
                                                         num_paths=num_paths, ts_step=deltaT, config=config,
                                                         device=device,
