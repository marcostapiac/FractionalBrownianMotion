...
bws = np.logspace(-3, -1.1, 20)#np.logspace(-4, -0.05, 20)  # np.logspace(-2, -0.05, 20)
...
for bw in bws:
    ...
    save_path = (
            project_config.ROOT_DIR + f"experiments/results/IIDNadaraya_fSinLog_DriftEvalExp_{round(bw, 6)}bw_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.log_space_scale}b_{config.sin_space_scale}c_{config.ts_length}NumDPS").replace(
        ".", "")
