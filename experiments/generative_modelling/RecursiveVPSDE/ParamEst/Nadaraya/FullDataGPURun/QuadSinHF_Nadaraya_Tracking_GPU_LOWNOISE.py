...
def true_drift_gpu(prev: torch.Tensor, num_paths: int, config) -> torch.Tensor:
    ...
    drift_X = -2. * config.quad_coeff * prev + config.sin_coeff * config.sin_space_scale * torch.sin(
...
if __name__ == "__main__":
    ...
    with ResourceLogger(
        ...
        for bw_idx in tqdm(range(0,bws.shape[0])):
            ...
            save_path = (
                    project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fQuadSinHF_DriftTrack_{round(bw[0], 6)}bw_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quad_coeff}a_{config.sin_coeff}b_{config.sin_space_scale}c_{config.ts_length}NumDPS_Diff{config.diffusion:.1f}").replace(
                ".", "")
            ...
            Xs = torch.linspace(-.15, .15, all_true_states.shape[0])[:, np.newaxis].to(device)
        ...
        save_path = (
                project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fQuadSinHF_DriftEvalExp_MSEs_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quad_coeff}a_{config.sin_coeff}b_{config.sin_space_scale}c_{config.ts_length}NumDPS_Diff{config.diffusion:.1f}").replace(
            ".", "")
