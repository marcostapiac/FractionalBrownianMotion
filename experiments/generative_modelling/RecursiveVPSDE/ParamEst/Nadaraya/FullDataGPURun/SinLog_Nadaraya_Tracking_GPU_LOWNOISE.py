...
def true_drift_gpu(prev: torch.Tensor, num_paths: int, config) -> torch.Tensor:
    ...
    drift_X = (-torch.sin(config.sin_space_scale * prev) * torch.log(
        1 + config.log_space_scale * torch.abs(prev)) / config.sin_space_scale)
...
if __name__ == "__main__":
    ...
    with ResourceLogger(
        ...
        for bw_idx in tqdm(range(0, bws.shape[0])):
            ...
            save_path = (
                    project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fSinLog_DriftTrack_{round(bw[0], 6)}bw_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.log_space_scale}b_{config.sin_space_scale}c_{config.ts_length}NumDPS_Diff{config.diffusion:.1f}").replace(
                ".", "")
        ...
        save_path = (
                project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fSinLog_DriftEvalExp_MSEs_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.log_space_scale}b_{config.sin_space_scale}c_{config.ts_length}NumDPS_Diff{config.diffusion:.1f}").replace(
            ".", "")
