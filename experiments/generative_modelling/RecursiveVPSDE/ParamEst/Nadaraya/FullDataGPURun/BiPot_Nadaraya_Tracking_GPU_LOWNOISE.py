...
if __name__ == "__main__":
    ...
    with ResourceLogger(
        ...
        for bw_idx in tqdm(range(0, bws.shape[0])):
            ...
            save_path = (
                    project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fBiPot_DriftTrack_{round(bw[0], 6)}bw_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_Diff{config.diffusion:.1f}").replace(
                ".", "")
            ...
            Xs = torch.linspace(-0.3, 0.3, all_true_states.shape[0])[:, np.newaxis].to(device)
        ...
        save_path = (
                project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fBiPot_DriftEvalExp_MSEs_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_Diff{config.diffusion:.1f}").replace(
            ".", "")
