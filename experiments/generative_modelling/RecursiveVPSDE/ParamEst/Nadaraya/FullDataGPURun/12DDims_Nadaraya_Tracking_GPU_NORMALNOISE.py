...
from configs.RecursiveVPSDE.Markovian_fBiPotDDims.recursive_Markovian_PostMeanScore_fBiPot12Dims_T256_H05_tl_110data_StbleTgt_FULLDATA_NORMALNOISE import \
    get_config
...
if __name__ == "__main__":
    ...
    with ResourceLogger(
        ...
        for bw_idx in tqdm(range(0, bws.shape[0])):
            ...
            save_path = (
                    project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fBiPot_{config.ndims}DDims_DriftTrack_{round(bw[0], 6)}bw_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff[0]}a_{config.quad_coeff[0]}b_{config.const[0]}c_Diff{config.diffusion:.1f}").replace(
                ".", "")
        ...
        save_path = (
                project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fBiPot_{config.ndims}DDims_DriftEvalExp_MSEs_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff[0]}a_{config.quad_coeff[0]}b_{config.const[0]}c_Diff{config.diffusion:.1f}").replace(
            ".", "")
