...
if __name__ == "__main__":
    ...
    with ResourceLogger(
        ...
        save_path = (
                project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fQuadSinHF_DriftEvalExp_MSEs_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.quad_coeff}a_{config.sin_coeff}b_{config.sin_space_scale}c_{config.ts_length}NumDPS").replace(
            ".", "")
