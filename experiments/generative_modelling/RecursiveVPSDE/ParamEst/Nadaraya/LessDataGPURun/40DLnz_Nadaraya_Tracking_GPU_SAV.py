...
if __name__ == "__main__":
    ...
    same_rndm_idxs = np.arange(num_state_paths)
    ...
    save_path = (
        ...
        ".", "")
    mses = pd.DataFrame.from_dict(mses, orient="index", columns=["bw", "mse"]).sort_values("bw").set_index("bw")
    print(mses.index[np.argmin(mses.values).flatten()])