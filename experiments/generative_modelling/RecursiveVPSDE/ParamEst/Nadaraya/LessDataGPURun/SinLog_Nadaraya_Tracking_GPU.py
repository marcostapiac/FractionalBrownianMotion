...
if __name__ == "__main__":
    ...
    with ResourceLogger(
        ...
        for bw_idx in tqdm(range(0,bws.shape[0])):
            ...
            save_path = (
                    project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fSinLog_DriftTrack_{round(bw[0], 6)}bw_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.log_space_scale}b_{config.sin_space_scale}c_{config.ts_length}NumDPS").replace(
                ".", "")
            ...
            Xs = torch.linspace(-1.5, 1.5, 256)[:, np.newaxis].to(device)
        ...
        save_path = (
                project_config.ROOT_DIR + f"experiments/results/IIDNadarayaGPU_fSinLog_DriftEvalExp_MSEs_{num_paths}NPaths_{config.t0}t0_{config.deltaT:.3e}dT_{config.log_space_scale}b_{config.sin_space_scale}c_{config.ts_length}NumDPS").replace(
            ".", "")
