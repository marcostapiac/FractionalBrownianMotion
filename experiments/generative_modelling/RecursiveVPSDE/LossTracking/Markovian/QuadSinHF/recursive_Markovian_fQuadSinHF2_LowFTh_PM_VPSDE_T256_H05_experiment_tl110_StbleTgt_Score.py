...
from src.classes.ClassConditionalStbleTgtMarkovianDiffTrainer import \
    ConditionalStbleTgtMarkovianDiffTrainer
...
if __name__ == "__main__":
    # Data parameters
    from configs.RecursiveVPSDE.Markovian_fQuadSinHF.recursive_Markovian_Score_fQuadSinHF2_LowFTh_T256_H05_tl_110data_StbleTgt import get_config
    config = get_config()
    with ResourceLogger(
            interval=120,
        ...
        try:
            ...
            train_and_save_recursive_diffusion_model(data=data, config=config, diffusion=diffusion, scoreModel=scoreModel,
                                                     trainClass=ConditionalStbleTgtMarkovianDiffTrainer)
        cleanup_experiment()
