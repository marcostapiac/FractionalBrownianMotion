...
if __name__ == "__main__":
    # Data parameters
    from configs.RecursiveVPSDE.LSTM_fBiPot.recursive_LSTM_PostMeanScore_fBiPot_T256_H05_tl_110data_WRMSE import \
        get_config
    ...
    try:
        ...
        try:
            ...
            data = generate_fBiPot(num_dims=config.ndims, config=config,T=config.ts_length, isUnitInterval=config.isUnitInterval, S=training_size,
        ...
        train_and_save_recursive_diffusion_model(data=data, config=config, diffusion=diffusion, scoreModel=scoreModel,
                                                 trainClass=ConditionalLSTMPostMeanDiffTrainer)
    cleanup_experiment()
