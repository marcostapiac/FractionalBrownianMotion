...
true_paths = np.load(config_postmean.data_path, allow_pickle=True)[:10,:]
...
with torch.no_grad():
    ...
    fixed_feature = output[[0], [123], :].unsqueeze(0)
    print(fixed_feature.shape)
    assert(fixed_feature.shape == (1,1,20))
    fixed_feature = torch.tile(fixed_feature,dims=(N, 1,1))
del features, true_paths, output
...
with torch.no_grad():
    ...
    for diff_index in tqdm(range(0, config_postmean.max_diff_steps - 9100)):
        ...
        if diff_index == 900-1:
            ...
            print(((diffusion_var + diffusion_mean2*ts_step)/(diffusion_mean*ts_step)))
        ...
        x = drift + diffParam * z
print(drift_est.shape, drift_est.shape)