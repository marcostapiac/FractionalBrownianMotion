...
def estimate_SDEs(config:ConfigDict, train_epoch:int)->None:
    paths = pd.read_csv(config.experiment_path + "_NEp{}.csv.gzip".format(train_epoch), compression="gzip", index_col=[0,1])
    # Estimate Hurst indices from paths
    U_a1, U_a2 = second_order_estimator(paths=paths.to_numpy(), Nsamples=paths.shape[0])
