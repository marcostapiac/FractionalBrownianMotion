...
def TSPM_drift_eval():
    ...
    if "fOU" in config.data_path:
        ...
        save_path = (
                    project_config.ROOT_DIR + f"experiments/results/TSPMS_ES{es}_fSin_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.mean_rev}MeanRev_{config.max_diff_steps}DiffSteps").replace(
            ".", "")
