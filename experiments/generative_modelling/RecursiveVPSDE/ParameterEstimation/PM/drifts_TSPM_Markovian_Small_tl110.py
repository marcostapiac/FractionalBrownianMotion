...
if "fOU" in config.data_path:
    save_path = \
        (
                project_config.ROOT_DIR + f"experiments/results/TSPM_Small110_mkv_ES{es}_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.mean}Mean_{config.max_diff_steps}DiffSteps").replace(
            ".", "")
elif "fSin" in config.data_path:
    save_path = (
            project_config.ROOT_DIR + f"experiments/results/TSPM_Small110_mkv_ES{es}_fSin_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.mean_rev}MeanRev_{config.max_diff_steps}DiffSteps").replace(
        ".", "")
print(save_path)
