...
print(type)
save_path = (
            project_config.ROOT_DIR + f"experiments/results/TS_mkv_fBiPot_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_{config.max_diff_steps}DiffSteps").replace(
        ".", "")
