...
import numpy as np
import torch
from configs.RecursiveVPSDE.Markovian_fBiPot.recursive_Markovian_fBiPotWithPosition_T256_H05_tl_5data import get_config as get_config
...
if "fOU" in config.data_path:
    ...
    save_path = (
            project_config.ROOT_DIR + f"experiments/results/TS_mkv_ES{es}_fBiPot_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.mean_rev}MeanRev_{config.max_diff_steps}DiffSteps").replace(
        ".", "")
