...
print(type)
save_path = (
        project_config.ROOT_DIR + f"experiments/results/TSPM_mkv_fBiPot_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_{config.deltaT:.3e}dT_{config.beta_max:.1e}betaMax").replace(
    ".", "")
print(save_path)
