...
if __name__ == "__main__":
    ...
    for Nepoch in config.max_epochs:
        try:
            ...
            save_path = (
                    project_config.ROOT_DIR + f"experiments/results/TSPM_LSTM_fBiPot_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_{config.beta_max:.1e}betaMax_{config.loss_factor}LFac").replace(
                ".", "")
            print(save_path)
