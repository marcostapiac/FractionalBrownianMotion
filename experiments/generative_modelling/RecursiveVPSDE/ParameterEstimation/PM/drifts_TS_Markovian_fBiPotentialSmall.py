...
import numpy as np
import torch
from configs.RecursiveVPSDE.Markovian_fBiPotSmall.recursive_Markovian_fBiPotSmallWithPosition_T256_H05_tl_110data import get_config as get_config
...
print(type)
save_path = (
            project_config.ROOT_DIR + f"experiments/results/TS_mkv_ES{es}_fBiPotSmall_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_{config.max_diff_steps}DiffSteps").replace(
        ".", "")
