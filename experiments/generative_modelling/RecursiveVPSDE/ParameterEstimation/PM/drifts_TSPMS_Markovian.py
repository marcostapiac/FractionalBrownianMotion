...
import numpy as np
import torch
from configs.RecursiveVPSDE.Markovian_fSin.recursive_Markovian_PostMeanScaledScore_fSin_T256_H05_tl_5data import \
...
while difftime_idx >= 0:
    ...
    final_vec_mu_hats[:, difftime_idx, :] = final_mu_hats.reshape((num_taus, Xshape)).T.numpy()
...
def plot_drift_estimator(mean, stds, numpy_Xs, type, toSave: bool = True):
    ...
    if toSave:
        plt.savefig(
            f"/Users/marcos/Library/CloudStorage/OneDrive-ImperialCollegeLondon/StatML_CDT/Year2/DiffusionModelPresentationImages/fSin_{type}.png")
    plt.show()
    plt.close()
...
if "fOU" in config.data_path:
    save_path = \
        (
                project_config.ROOT_DIR + f"experiments/results/TSPMS_mkv_ES{es}_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.mean}Mean_{config.max_diff_steps}DiffSteps").replace(
            ".", "")
elif "fSin" in config.data_path:
    save_path = (
            project_config.ROOT_DIR + f"experiments/results/TSPMS_mkv_ES{es}_fSin_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.mean_rev}MeanRev_{config.max_diff_steps}DiffSteps").replace(
        ".", "")
