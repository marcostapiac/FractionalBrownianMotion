...
Xs = torch.linspace(-1.5, 1.5, steps=Xshape).unsqueeze(-1).unsqueeze(-1).permute(1, 0, 2).to(device)
...
print(type)
save_path = (
        project_config.ROOT_DIR + f"experiments/results/TSPM_mkv_fBiPotSmall_DriftEvalExp_{Nepoch}Nep_{config.loss_factor}LFactor_{config.t0}t0_{config.deltaT:.3e}dT_{config.quartic_coeff}a_{config.quad_coeff}b_{config.const}c_{config.beta_max:.1e}betaMax_{config.loss_factor}LFac").replace(
    ".", "")
print(save_path)
