...
def sig_path_score_feature_analysis() -> None:
    ...
    for train_epoch in [960]:  # config.max_epochs:
        paths = pd.read_parquet(
            "/Users/marcos/GitHubRepos/FractionalBrownianMotion/experiments/results/rec_TSM_False_incs_True_unitIntv_fBm_VPSDE_model_H7000e-01_T256_Ndiff10000_Tdiff1000e+00_trainEps1e-04_BetaMax20000e+01_BetaMin10000e-04_DiffEmbSize64_ResLay10_ResChan8_DiffHiddenSize64_TrueHybrid_TrueWghts_Sig_Trunc5_Dim2_tl5_NEp960_SFS.parquet.gzip".format(
                train_epoch), engine="pyarrow")
        good_feat_df = pd.read_parquet(
            "/Users/marcos/GitHubRepos/FractionalBrownianMotion/experiments/results/feature_data/fBm_H07_T256_SigTrunc5_SigDim2_NEp{}_SFS_good.parquet.gzip".format(
                train_epoch), engine="pyarrow")
        bad_feat_df = pd.read_parquet(
            "/Users/marcos/GitHubRepos/FractionalBrownianMotion/experiments/results/feature_data/fBm_H07_T256_SigTrunc5_SigDim2_NEp{}_SFS_bad1.parquet.gzip".format(
                train_epoch), engine="pyarrow")
        true_feat_df = pd.read_parquet(
        ...
        avg_true_feat_df = np.mean(
            true_feat_df.to_numpy().reshape((config.ts_length, true_feat_df.index.levshape[1], feat_dim)), axis=1)
        avg_good_feat_df = np.mean(
            good_feat_df.to_numpy().reshape((config.ts_length, good_feat_df.index.levshape[1], feat_dim)), axis=1)
        avg_bad_feat_df = np.mean(
            bad_feat_df.to_numpy().reshape((config.ts_length, bad_feat_df.index.levshape[1], feat_dim)), axis=1)
        assert (avg_true_feat_df.shape == (config.ts_length,
                                           feat_dim) and avg_good_feat_df.shape == avg_true_feat_df.shape and avg_bad_feat_df.shape == avg_true_feat_df.shape)
        ...
        good_paths = paths.loc[bad_idxs, :]
        hurst_estimation(good_paths.to_numpy(), sample_type="Final Time Samples at Train Epoch {}".format(train_epoch),
