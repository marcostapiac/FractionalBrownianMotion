...
def run_experiment(dataSize: int, diffusion: VPSDEDiffusion, scoreModel: Union[NaiveMLP, TimeSeriesScoreMatching],
    ...
    return evaluate_fBm_performance(true_samples, fBm_samples.cpu().numpy(), rng=rng, config=config, exp_dict=experiment_res)
...
if __name__ == "__main__":
    ...
    for j in range(1, config.num_runs + 1):
        ...
        exp_dict = run_experiment(diffusion=diffusion, rng=rng, scoreModel=scoreModel, dataSize=s, config=config, experiment_res=exp_dict)
        agg_dict[j] = exp_dict
