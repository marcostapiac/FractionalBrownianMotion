...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassVESDEDiffusion, sampleEps: float,
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            file = open(
                config.ROOT_DIR + "src/generative_modelling/trained_models/trained_CEV_VESDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                    td,
                    N, trainEps),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_modelling/trained_models/trained_CEV_VESDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                                                       td,
                                                       N, trainEps),
