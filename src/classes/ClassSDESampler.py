...
class SDESampler:
    ...
    def __init__(self, diffusion: Union[VESDEDiffusion, VPSDEDiffusion, OUDiffusion], sample_eps: float,
    ...
    def sample(self, shape:Tuple[int,int])->torch.Tensor:
        ...
        for i in tqdm(iterable=(range(0, self.predictor.max_diff_steps)), dynamic_ncols=False, desc="Sampling :: ", position=0):
            x, pred_score, noise = self.predictor.step(x, t=timesteps[i] * torch.ones((x.shape[0], 1)),
                                                       diff_index=i)  # device = TODO
            if type(self.corrector) != NoneType:
                x = self.corrector.sample(x, predicted_score=pred_score, predictor_noise=noise, diff_index=i)
        return x
