...
class ConditionalDiffusionModelTrainerLossTracker:
    ...
    def _save_loss(self, filepath:str):
        """
        Save loss tracker
        ...
        """
        import pickle
        with open(filepath.replace("/trained_models/","/training_losses/") +"_loss", 'wb') as fp:
        ...
        for epoch in range(self.epochs_run, max_epochs):
            ...
            if self.device_id == 0 or type(self.device_id) == torch.device:
                if epoch + 1 == max_epochs:
                    ...
                    self._save_loss(filepath=model_filename)
