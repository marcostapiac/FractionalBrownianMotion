...
class VPSDEDiffusion(nn.Module):
    def __init__(self, device: torch.cuda.Device, model, numDiffSteps: int, trainEps: float, noiseFactor: float,
    ...
    def get_beta_min(self):
        return 0.0001 * self.noiseFactor  # *1.8 for fBm, *1 for circle, *1.85 for fBn

    def get_beta_max(self):
        return 10. * self.noiseFactor
    ...
    def one_epoch_diffusion_train(self, opt: torch.optim.Optimizer,
        ...
        for x0s in iter(trainLoader):  # Iterate over batches (training data is already randomly selected)
            ...
            pred = self.model.forward(xts, effTimes.squeeze(-1))
    ...
    def reverse_process(self, data: np.ndarray, dataSize: int, timeDim: int, reverseTimes: torch.Tensor,
                        sigNoiseRatio: float, numLangevinSteps:int=10,
        ...
        with torch.no_grad():
            for i in tqdm(iterable=(range(timeLim, self.numDiffSteps)), dynamic_ncols=False,
                ...
                effTimes = (0.5 * ts ** 2 * (self.get_beta_max() - self.get_beta_min()) + ts * self.get_beta_min())
                beta_t = self.betas[self.numDiffSteps - 1 - i]  # dt absorbed already
                predicted_score = self.model.forward(x, effTimes.squeeze(-1)).squeeze(1)  # Score == Noise/STD!
                #true_score = (-x + np.exp(-0.5 * effTimes) * data) / (1. - np.exp(-effTimes))
                #print(predicted_score, true_score)
                #scoreDiff = predicted_score - true_score
                #print("Mean difference in scores:", i, ts[0],torch.mean(torch.pow(scoreDiff, 2.)))
                z = torch.randn_like(x)
                x = x*(2.-torch.sqrt(1.-beta_t)) + beta_t*predicted_score + np.sqrt(beta_t) * z
                # 1.+0.5*beta_t
                for _ in range(numLangevinSteps):
                    z = torch.randn_like(x)
                    e = 2. * self.alphas[self.numDiffSteps - 1 - i] * (sigNoiseRatio * np.linalg.norm(z) / np.linalg.norm(predicted_score)) ** 2
