...
class OUDiffusion(nn.Module):
    def __init__(self, device: torch.cuda.Device, model: ClassTimeSeriesNoiseMatching, N: int, trainEps: float,
    ...
    def reverse_process(self, dataSize: int, timeDim: int, reverseTimes: torch.Tensor, timeLim: int = 0) -> np.ndarray:
        """ Reverse process for D datapoints of size T """
        assert (reverseTimes[-1] >= self.trainEps and reverseTimes.shape[
            0] == self.numDiffSteps)  # Ensure we don't sample from times we haven't seen during training
