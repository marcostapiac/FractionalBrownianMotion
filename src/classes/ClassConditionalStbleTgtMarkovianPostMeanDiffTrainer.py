...
class ConditionalStbleTgtMarkovianPostMeanDiffTrainer(nn.Module):
    ...
    def _batch_loss_compute(self, outputs: torch.Tensor, targets: torch.Tensor,  w_tau, epoch: int,
        ...
        print(f"MSE per Dim at Epoch {epoch}, {torch.round(base_loss, decimals=3)}\n")
        base_loss = 0.5 * (base_loss * torch.exp(-self.s)).sum() + 0.5 * self.s.sum()
        print(f"MSE per Dim after Weighting at Epoch {epoch}, {torch.round(base_loss,decimals=3)}\n")
        var_loss = ((
