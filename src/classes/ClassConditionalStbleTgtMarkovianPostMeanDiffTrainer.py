...
class ConditionalStbleTgtMarkovianPostMeanDiffTrainer(nn.Module):
    ...
    def _batch_loss_compute(self, outputs: torch.Tensor, targets: torch.Tensor, epoch:int, batch_idx:int, num_batches:int) -> float:
        ...
        loss += 0.0001*torch.mean(self.score_network.module.mlp_state_mapper.hybrid.log_scale**2)
    ...
    def _load_snapshot(self, snapshot_path: str, config) -> None:
        ...
        try:
            ...
            if "QuadSin" in config.data_path and config.sin_space_scale == 25. and self.opt.param_groups[0]["lr"] == 0.001 and config.feat_thresh == 1./50. and 250<= self.epochs_run <= 300 : self.opt.param_groups[0]["lr"]=0.0001
