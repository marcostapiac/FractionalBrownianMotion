...
class ConditionalStbleTgtMarkovianPostMeanDiffTrainer(nn.Module):
    ...
    def train(self, max_epochs: list, model_filename: str, batch_size: int, config) -> None:
        ...
        for epoch in range(self.epochs_run, end_epoch):
            ...
            ratio = (0.2 * average_base_loss_per_epoch) / ((1. - 0.2) * average_var_loss_per_epoch + 1e-12)
            self.var_loss_reg = min(ratio, 0.1)

            ratio = (0.15 * average_base_loss_per_epoch) / ((1. - 0.15) * average_mean_loss_per_epoch + 1e-12)
