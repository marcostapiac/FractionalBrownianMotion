...
class ConditionalStbleTgtMarkovianPostMeanDiffTrainer(nn.Module):
    ...
    def train(self, max_epochs: list, model_filename: str,batch_size: int, config) -> None:
        ...
        for epoch in range(self.epochs_run, end_epoch):
            ...
            if epoch % 20 == 0:
                tau = max(self.score_network.module.mlp_state_mapper.hybrid.final_tau, self.score_network.module.mlp_state_mapper.hybrid.init_tau * (0.85 ** (epoch//20)))
                self.score_network.mlp_state_mapper.module.hybrid.set_tau(tau)
