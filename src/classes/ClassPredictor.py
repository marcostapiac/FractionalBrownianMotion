...
class Predictor(abc.ABC):
    ...
    def __init__(self, diffusion: Union[VESDEDiffusion, VPSDEDiffusion, OUSDEDiffusion],
                 score_function: Union[NaiveMLP, TimeSeriesScoreMatching], end_diff_time: float, max_diff_steps: int, device:Union[int, torch.device]):
        super().__init__()
...
class EulerMaruyamaPredictor(Predictor):
    ...
    def __init__(self, diffusion: Union[VESDEDiffusion, VPSDEDiffusion, OUSDEDiffusion],
                 score_function: Union[NaiveMLP, TimeSeriesScoreMatching], end_diff_time: float, max_diff_steps: int, device:Union[int, torch.device]):
        super().__init__(diffusion, score_function, end_diff_time, max_diff_steps, device)

    def step(self, x_prev: torch.Tensor, t: torch.Tensor, diff_index: torch.Tensor) -> Tuple[
        torch.Tensor, torch.Tensor, torch.Tensor]:
        dt = -self.end_diff_time / self.max_diff_steps
        score, drift, diffusion = self.diffusion.get_reverse_sde(x_prev, score_network=self.score_network, t=t, dt=torch.Tensor([dt]).to(self.torch_device))
...
class AncestralSamplingPredictor(Predictor):
    ...
    def step(self, x_prev: torch.Tensor, t: torch.Tensor, diff_index: torch.Tensor) -> Tuple[
        torch.Tensor, torch.Tensor, torch.Tensor]:
        score, drift, diffusion = self.diffusion.get_ancestral_sampling(x_prev, t=t, score_network=self.score_network,diff_index=diff_index, max_diff_steps=self.max_diff_steps)
        z = torch.randn_like(x_prev)
        return drift + diffusion * z, score, z
