...
class VESDEDiffusion(nn.Module):
    ...
    def __init__(self, device: torch.cuda.Device, model, rng: np.random.Generator,
                 numDiffSteps: int, trainEps: float, noiseFactor: float, *args, **kwargs) -> None:
    ...
    def get_var_max(self) -> torch.Tensor:
        return torch.Tensor([(1. * self.noiseFactor) ** 2]).to(torch.float32)

    def get_var_min(self) -> torch.Tensor:
        return torch.Tensor([(0.01 * self.noiseFactor) ** 2]).to(torch.float32)

    def forward_process(self, dataSamples: torch.Tensor, diffusionTimes: torch.Tensor):
        epsts = torch.randn_like(dataSamples)
        vars = self.get_var_min() * (self.get_var_max() / self.get_var_min()) ** diffusionTimes
        return dataSamples + torch.sqrt(vars) * epsts, -epsts / torch.sqrt(vars)
    ...
    def get_loss_weighting(self, diffTimes: torch.Tensor) -> torch.Tensor:
        return torch.sqrt(self.get_var_min() * (self.get_var_max() / self.get_var_min()) ** diffTimes)
    ...
    def one_epoch_diffusion_train(self, opt: torch.optim.Optimizer,
        ...
        for x0s in iter(trainLoader):  # Iterate over batches (training data is already randomly selected)
            ...
            effTimes = (self.get_var_min() * (self.get_var_max() / self.get_var_min()) ** diffTimes).view(x0s.shape[0],
                                                                                                          *([1] * len(
                                                                                                              x0s.shape[
                                                                                                              1:])))

            pred = self.model.forward(xts, effTimes)  # self.model.forward(xts, diffTimes)
            ...
            for x0s in iter(loader):  # Iterate over batches (training data is already randomly selected)
                ...
                effTimes = (self.get_var_min() * (self.get_var_max() / self.get_var_min()) ** diffTimes).view(
                    ...
                    *([1] * len(
                        x0s.shape[
                        1:])))
                pred = self.model.forward(xts, effTimes)
                loss = self.training_loss_fn(
                    weighted_predicted=self.get_loss_weighting(diffTimes) * pred.squeeze(1),
                    weighted_true=self.get_loss_weighting(diffTimes) * true_score)
    ...
    def reverse_process(self, data: np.ndarray, dataSize: int, timeDim: int, reverseTimes: torch.Tensor,
        ...
        with torch.no_grad():
            for i in tqdm(iterable=(range(timeLim, self.numDiffSteps)), dynamic_ncols=False,
                ...
                effTimes = (self.get_var_min() * (self.get_var_max() / self.get_var_min()) ** ts)

                dt = 1. / self.numDiffSteps
                predicted_score = self.model.forward(x, effTimes).squeeze(1)  # Score == Noise/STD!-(x - data) / (effTimes)
                vars_t = self.vars[self.numDiffSteps - 1 - i] * 2. * torch.log(
                    self.get_var_max() / self.get_var_min()) * dt
                z = torch.randn_like(x)
                x = x + (vars_t * predicted_score) + torch.sqrt(vars_t) * z

                for _ in range(numLangevinSteps):
                    z = torch.randn_like(x)
                    #predicted_score = self.model.forward(x.unsqueeze(1), effTimes).squeeze(1) # Only for corrector only
