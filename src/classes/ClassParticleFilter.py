...
from utils.math_functions import logsumexp
...
class FractionalParticleFilter(ParticleFilter):
    ...
    def particle_move(self, particle, currObs, deltaT, rho=0.1):
        ...
        # TODO: Include acceptance ratio since we need the likelihood ratio
        ...
        logAccProb = smultnorm.logpdf(currObs[1:], mean=currObs[:N] + muU * deltaT - 0.5 * deltaT * np.exp(Xnew[1:]),
                                      cov=deltaT * np.exp(0.5 * np.reshape(Xnew[1:], (N, 1))) @ np.exp(
                                          0.5 * np.reshape(Xnew[1:], (N, 1)).T))
        logAccProb -= smultnorm.logpdf(currObs[1:], mean=currObs[:N] + muU * deltaT - 0.5 * deltaT * np.exp(Xcurr[1:]),
                                       cov=deltaT * np.exp(0.5 * np.reshape(Xcurr[1:], (N, 1))) @ np.exp(
                                           0.5 * np.reshape(Xcurr[1:], (N, 1)).T))
