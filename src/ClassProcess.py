...
class FractionalBrownianNoise:
    ...
    def spectral_helper(self, x):
        ...
        B3 = np.squeeze(np.sum(
            [np.power((2. * np.pi * j + x), -2. * self.H - 1) + np.power((2. * np.pi * j - x), -2. * self.H - 1) for j in
             range(1, 4)]))
        B3 += np.squeeze(np.sum(
            [np.power((2. * np.pi * j + x), -2. * self.H) + np.power((2. * np.pi * j - x), -2. * H) for j in
             range(3, 5)])) / (
                      8. * self.H * np.pi)
    ...
    def davies_and_harte_simulation(self, N_samples):
        ...
        c = np.append(c, 0.)
        c = np.append(c, np.squeeze(np.array([self.covariance(2 * N_samples - 1 - j) for j in range(N_samples)])))
        lambdas = np.fft.fft(c)
        dotPs = np.diag(np.atleast_2d(np.sqrt(lambdas)).T @ W.T)
        Zs = np.fft.ifft(dotPs)  # TODO: Check implementation divides by root of length of dotPs
        return np.real(Zs[:N_samples])

    def crmd_simulaton(self, N_samples, l, r):
        g = int(np.ceil(np.log2(N_samples)))
        Z01 = np.random.normal()
        Zprev = [Z01]
        for i in range(1, g + 1):
            Zcurr = []
            expConst = 2 ** (i - 1)
            expHConst = 2 ** (-2 * self.H * i)
            upper = np.atleast_2d([2 ** (2 * self.H)])
            ...
            for j in range(expConst):
                ...
                c = np.atleast_2d(np.hstack([[self.covariance((2 * j + 1) - 2 * n) + self.covariance(
                    (2 * j + 1) - 2 * (n) + 1) for n in right_indices],
    ...
    def paxon_simulation(self, N_samples):
        ...
        fourier_coeffs = np.append(fourier_coeffs, np.sqrt(self.spectral_density(np.pi) / (2. * N_samples)) * np.random.normal())
