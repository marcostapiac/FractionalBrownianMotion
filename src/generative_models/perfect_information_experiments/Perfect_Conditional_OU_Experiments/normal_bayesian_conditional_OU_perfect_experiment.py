...
if __name__ == "__main__":
    ...
    for i in tqdm((range(N))):
        ...
        observed_score = -torch.cat([torch.from_numpy(np.linalg.inv(covY+((torch.exp(t) - 1.)*0.01*np.eye(trial_data.shape[1])).numpy())) @ (
                observed[s].reshape((2, 1)) - x0_est[s].reshape((2, 1)) - 12. * torch.ones_like(x0_est[s]).reshape(
    ...
    plot_diffusion_marginals(posteriors.numpy(), generated_samples, timeDim=2, diffTime=0)
    plot_diffusion_marginals(latents.numpy(), generated_samples, timeDim=2, diffTime=0)
