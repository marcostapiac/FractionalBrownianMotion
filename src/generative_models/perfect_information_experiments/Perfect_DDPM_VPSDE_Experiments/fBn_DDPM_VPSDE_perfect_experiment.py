...
if __name__ == "__main__":
    ...
    beta_min = 0.1
    beta_max = 20.
    betas = np.linspace(start=beta_min, stop=beta_max, num=N)  # Discretised noise schedule
    timesteps = np.linspace(start=1e-3, stop=1., num=N)  # Discretised time axis
    ...
    for i in tqdm(range(N)):
        ...
        xts = np.exp(-0.5 * beta_t * t) * x0s + np.sqrt(1. - np.exp(-beta_t * t)) * epsts
    ...
    reversed_timesteps = torch.linspace(start=1., end=1e-2, steps=N)
