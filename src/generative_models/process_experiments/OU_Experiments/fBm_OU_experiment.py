...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassOUDiffusion,
    ...
    fBm_samples = diffusion.reverse_process(dataSize=dataSize, data=data, timeDim=timeDim, timeLim=0,sampleEps=sampleEps)
    true_samples = generate_fBm(H=hurst, T=timeDim, S=dataSize, rng=rng)
    evaluate_fBm_performance(true_samples, fBm_samples, h=hurst, td=timeDim, rng=rng, unitInterval=True, evalMarginals=True, annot=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            file = open(
                config.ROOT_DIR + "src/generative_models/models/trained_fBm_OU_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}".format(
                    td,
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_models/models/trained_fBm_OU_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}".format(
                    td,
                    N, int(Tdiff), trainEps),
