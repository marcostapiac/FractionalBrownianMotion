...
mlp_model = pickle.load(open(config.ROOT_DIR + "src/generative_models/models/trained_MLP_noisy_circle_OU_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}_TembDim{}_EncShapes{}".format(
                        td,
                        N, Tdiff, trainEps ,temb_dim, enc_shapes), 'rb'))
tsm_model = pickle.load(open(config.ROOT_DIR + "src/generative_models/models/trained_TSM_noisy_circle_OU_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}_DiffEmbSize{}_ResidualLayers{}_ResChan{}_DiffHiddenSize{}".format(
                        td,
...
# Plot MSE errors
fig, ax = plt.subplots()
ax.plot(reverse_times, np.cumsum(np.sqrt(MLP_MSEs))-np.cumsum(np.sqrt(TSM_MSEs)), label="Difference in Cumulative RMSEs")
