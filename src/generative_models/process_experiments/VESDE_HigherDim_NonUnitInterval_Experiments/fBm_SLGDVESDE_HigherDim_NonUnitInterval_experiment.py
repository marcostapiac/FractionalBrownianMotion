...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassVESDEDiffusion,
    ...
    evaluate_fBm_HigherDim_performance(true_samples, fBm_samples, td=timeDim, h=hurst,rng=rng)
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            file = open(
                config.ROOT_DIR + "src/generative_models/models/trained_fBm_VESDE_NUI_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                    td,
                    N, trainEps),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_models/models/trained_fBm_VESDE_NUI_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                                                       td,
                                                       N, trainEps),
