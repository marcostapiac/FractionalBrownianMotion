...
from utils.data_processing import save_and_train_diffusion_model, evaluate_SDE_performance, \
    evaluate_SDE_HigherDim_performance
...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassVPSDEDiffusion,sampleEps: float, data: np.ndarray) -> None:
    assert (0. < hurst < 1.)
    ...
    cev_samples = diffusion.reverse_process(dataSize=dataSize, timeDim=timeDim, timeLim=0, data=data[:dataSize],
                                            ...
                                            sigNoiseRatio=.0005, numLangevinSteps=10)
    true_samples = generate_CEV(H=hurst, T=timeDim, S=10000, alpha=alpha, muX=muX,X0=X0,sigmaX=sigmaX, rng=rng, U0=U0, muU=muU)
    evaluate_SDE_HigherDim_performance(true_samples=true_samples, generated_samples=cev_samples, td=timeDim)
...
if __name__ == "__main__":
    ...
    try:
        data = np.load(config.ROOT_DIR + "data/two_hundred_thousand_CEV_samples_H{}_T{}_alpha{}_sigmaX{}_muX{}.npy".format(h,td, alpha, sigmaX, muX))
        data = data[:numSamples // 1, :]
        try:
            file = open(
                config.ROOT_DIR + "src/generative_models/models/trained_CEV_VPSDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                    td,
                    N, trainEps),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_models/models/trained_CEV_VPSDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                                                       td,
                                                       N, trainEps),
        ...
        data = generate_CEV(H=h, T=td, S=numSamples, rng=rng, X0=X0, U0=U0, alpha=alpha, sigmaX=sigmaX, muU=muU,
                            muX=muX)
        np.save(config.ROOT_DIR + "data/two_hundred_thousand_CEV_samples_H{}_T{}_alpha{}_sigmaX{}_muX{}.npy".format(h,td, alpha, sigmaX, muX), data)
        data = data[:numSamples // 1, :]
        # scoreModel = NaiveMLP(output_shape=td, enc_shapes=[32, 32], temb_dim=32, dec_shapes=[32, 32])
        scoreModel = TimeSeriesNoiseMatching()
        diffusion = VPSDEDiffusion(device="cpu", model=scoreModel, numDiffSteps=N, rng=rng, trainEps=trainEps,
    ...
    run_experiment(diffusion=model, hurst=h, timeDim=td, data=data, dataSize=1000, sampleEps=sampleEps)
