...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassVESDEDiffusion,
    ...
    fBm_samples = diffusion.reverse_process(dataSize=dataSize, timeDim=timeDim, timeLim=0, data=data,
                                            sampleEps=sampleEps,
                                            sigNoiseRatio=0.01, numLangevinSteps=0)  # 0.007 for good Chi2 test, 0.01 for good stats
    true_samples = generate_fBm(H=hurst, T=timeDim, S=dataSize, rng=rng)
    evaluate_fBm_performance(true_samples, fBm_samples, h=hurst, td=timeDim, rng=rng, unitInterval=True, annot=False, evalMarginals=True)
...
if __name__ == "__main__":
    ...
    mlpFileName = config.ROOT_DIR + "src/generative_models/models/trained_MLP_fBm_VESDE_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}_StdMax{:.4f}_StdMin{:.4f}_TembDim{}_EncShapes{}".format(
        td,
        N, Tdiff, trainEps, std_max, std_min, temb_dim, enc_shapes)
    tsmFileName = config.ROOT_DIR + "src/generative_models/models/trained_TSM_fBm_VESDE_model_T{}_Ndiff{}_Tdiff{}_trainEps{:.0e}_StdMax{:.4f}_StdMin{:.4f}_DiffEmbSize{}_ResidualLayers{}_ResChan{}_DiffHiddenSize{}".format(
        td,
