...
def run_experiment(timeDim: int, dataSize: int, diffusion: ClassVPSDEDiffusion, sampleEps: float,
    ...
    circle_samples = diffusion.reverse_process(dataSize=dataSize, timeDim=timeDim, timeLim=0,
                                               ...
                                               sigNoiseRatio=0.0075, numLangevinSteps=1)
...
if __name__ == "__main__":
    ...
    try:
        data = np.load(config.ROOT_DIR + "data/a_million_circle_samples_T{}.npy".format(td))[:numSamples // 5, :]
        try:
            file = open(
                config.ROOT_DIR + "src/generative_models/models/trained_circle_DDPMVPSDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                    td,
                    N, trainEps),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_models/models/trained_circle_DDPMVPSDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                                                       td,
                                                       N, trainEps),
