...
def run_experiment(hurst: float, timeDim: int, dataSize: int, diffusion: ClassVESDEDiffusion,sampleEps: float, data: np.ndarray) -> None:
    assert (0. < hurst < 1.)
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            file = open(
                config.ROOT_DIR + "src/generative_models/models/trained_CEV_VESDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                    td,
                    N, trainEps),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_models/models/trained_CEV_VESDE_model_T{}_Ndiff{}_trainEps{:.0e}".format(
                                                       td,
                                                       N, trainEps),
