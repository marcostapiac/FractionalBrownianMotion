...
def run_experiment(timeDim: int, dataSize: int, diffusion: ClassOUDiffusion) -> None:
    ...
    circle_samples = diffusion.reverse_process(dataSize=dataSize, timeDim=timeDim, timeLim=0,
                                               reverseTimes=torch.linspace(start=1., end=1e-3,
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            file = open(
                config.ROOT_DIR + "src/generative_models/models/trained_circle_OU_model_T{}_Ndiff{}".format(td, N),
                'rb')
            ...
            model = save_and_train_diffusion_model(data,
                                                   model_filename=config.ROOT_DIR + "src/generative_models/models/trained_circle_OU_model_T{}_Ndiff{}".format(
                                                       td, N),
    ...
    run_experiment(diffusion=model, timeDim=td, dataSize=1000)
