...
class MLPStateMapper(nn.Module):
    def __init__(self, ts_input_dim: int, hidden_dim: int, target_dims: int):
        ...
        self.feature_attn = FeatureSelfAttention(dim=ts_input_dim)

        self.linear2 = nn.Linear(ts_input_dim + hidden_dim + ts_input_dim + 2 * M, hidden_dim)
    ...
    def forward(self, x):
        ...
        x_attn = self.feature_attn(x)  # → [batch, D]

        x_fourier = self.hybrid(x)  # [batch, 2M]
        x_combined = torch.cat([x, x_raw, x_fourier, x_attn], dim=-1)  # [batch, D+hidden_dim + 2M]
...
class ConditionalMarkovianTSPostMeanScoreMatchingWithAttention(nn.Module):
    def __init__(
            self,
            max_diff_steps: int,
            diff_embed_size: int,
        ...
        self.mlp_state_mapper = MLPStateMapper(ts_input_dim=ts_dims, hidden_dim=mlp_hidden_dims,
                                               target_dims=condupsampler_length)
