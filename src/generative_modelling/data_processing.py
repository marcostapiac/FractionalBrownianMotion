...
def recursive_signature_reverse_sampling(diffusion: VPSDEDiffusion,
    ...
    with torch.no_grad():
        ...
        for t in range(config.ts_length):
            ...
            output = compute_current_sig_feature(ts_time=t, device=device,past_feat=output, basepoint=paths[max(t - 1,0)],latest_path=paths[max(0,t)], config=config, score_network=scoreModel, full_path=torch.concat(paths, dim=1))
